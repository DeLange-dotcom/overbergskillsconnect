...
function AdminApp() {
  const [tab, setTab] = useState<
    "apprentices" | "providers" | "opportunities" | "mentors" | "mentorship_requests" | "placements"
  >("apprentices");
  ...
  async function approve(table: string, id: string) {
    await supabase
      .from(table as never)
      .update({ approved: true, status: "approved" } as never)
      .eq("id", id);
  ...
  return (
    <SiteLayout>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        {loading ? (
          <div className="py-20 text-center text-brand-dark/50">
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-brand-dark/5">
                    ...
                    <td className="px-3 py-2">
                      {String(r.status ?? "—")}
                      {r.approved ? (
                        <span className="ml-2 text-brand-primary text-xs">✓ approved</span>
                      ) : null}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      ...
                      {tab === "mentors" && !r.approved && (
                        <button
                          onClick={() => approve("mentors", r.id)}
                          className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-brand-primary text-white"
                        >
                      ...
                      {tab === "providers" && !r.approved && r.status !== "approved" && (
                        <button
                          onClick={() => approve("apprenticeship_providers", r.id)}
                          className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-brand-primary text-white"
                        >
                          <CheckCircle className="size-3" /> Approve
                        </button>
                      ...
                      {tab === "opportunities" && !r.approved && (
                        <button
                          onClick={() => approveOpportunity(r.id)}
                          className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-brand-primary text-white"
                        >
